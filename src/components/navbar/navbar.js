...
import { NAVBAR_HOME, NAVBAR_SEARCH, NAVBAR_NONE } from './constants';
      ...
      <ul className="nav navbar-nav NavBar__LeftItems">
        {tabs.map(tab => (
          <NavBar.Element
            ...
            element={
              <Link className="nav-link" to={tab.to}>
                {tab.label}
              </Link>
            }
          />
      ...
      <ul className="nav navbar-nav pull-xs-right NavBar__RightItems">
        {user.getIn(['token', 'token']) ? (
          ...
          <li className="nav-item active">
            <button className="nav-link btn btn-link NavBar__LoginButton" onClick={onLogin}>
              Sign in
            </button>
          </li>
        )}
      </ul>
      <div className="nav nav-item navbar-nav pull-xs-right NavBar__Children">{children}</div>
    </div>
  </nav>
);

NavBar.propTypes = {
  activeTab: PropTypes.oneOf([NAVBAR_HOME, NAVBAR_SEARCH, NAVBAR_NONE]),
...
NavBar.Element = ({ active, element }) => {
  ...
  return <li className={classNames('nav-item', { active })}>{element}</li>;
};
